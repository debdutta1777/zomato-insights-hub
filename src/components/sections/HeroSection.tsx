  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-16">
      ...
      <div className="absolute inset-0">
        ...
        <div className="absolute inset-0 bg-zomato-dark/80" />
      </div>
      <div
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 text-center">
        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5 }} className="mb-6">
          <span className="font-display text-3xl md:text-4xl font-bold italic text-white/90 tracking-tight">zomato</span>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }} className="inline-flex items-center gap-2 px-5 py-2 rounded-full bg-white/10 backdrop-blur-md border border-white/15 text-white/80 text-sm font-medium mb-8">
          <span className="w-2 h-2 rounded-full bg-zomato-gold animate-pulse" />
          Zomathon 2025 — Hackathon Submission
        </motion.div>

        <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="text-5xl md:text-7xl lg:text-[5.5rem] font-display font-bold text-white leading-[1.05] mb-4">
          KPT Prediction
          <br />
          <span className="text-gradient-gold">&amp; Smart Dispatch</span>
        </motion.h1>

        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.35 }} className="text-lg md:text-xl text-white/60 max-w-2xl mx-auto mb-10">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.6 }} className="grid grid-cols-1 md:grid-cols-3 gap-px bg-white/10 rounded-2xl overflow-hidden max-w-3xl mx-auto">
          {[
            ...
            { value: `${performanceMetrics.p50Error.reduction}%`, label: "AVG WAIT IMPROVED" },
            ...
            <div key={i} className="bg-white/5 backdrop-blur-sm px-8 py-6 text-center">
              ...
              <div className="text-white/50 text-xs font-semibold tracking-widest uppercase">{stat.label}</div>
            </div>
          ))}
        </motion.div>
      </div>

      <motion.div animate={{ y: [0, 8, 0] }} transition={{ duration: 2, repeat: Infinity }} className="absolute bottom-8 left-1/2 -translate-x-1/2 text-white/40">
